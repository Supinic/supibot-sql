...
VALUES
	...
	switch (type) {
		case \"cookie\": {
			...
			else if (check.Cookie_Is_Gifted) {
				return { reply: \"That cookie was gifted to you! Eat it, don\'t give it away!\" };
			}

			const targetCheck = (await sb.Query.getRecordset(rs => rs
				.select(\"Cookie_Today\")
				.from(\"chat_data\", \"Extra_User_Data\")
				.where(\"User_Alias = %n\", targetUser.ID)
			))[0];
			if (targetCheck && !targetCheck.Cookie_Today) {
				return { reply: \"That user hasn\'t eaten their cookie today, so you would be wasting your gift! Get them to eat it!\" };
			}

			await sb.Query.raw([
				\"INSERT INTO chat_data.Extra_User_Data (User_Alias, Cookie_Today,  Cookie_Gifts_Sent)\", 
				\"VALUES (\" + extra.user.ID + \", 1, 1)\",
